<script lang="ts">
  ...
  import { DEFAULT_SETTINGS, TOOL_LABELS, type AppState, type AppSettings } from '../shared/types';
  ...
  async function action(fn: () => Promise<unknown>) {
    ...
    try { await writer.flush(); await fn(); } catch (e) { error = message(e); } finally { busy = false; }
  }
...
<div class="control-shell">
  <aside class="sidebar">
    ...
    <div class="sidebar-bottom"><span class="status-dot" class:drawing={appState?.mode === 'draw'}></span>{appState?.mode === 'draw' ? '그리는 중' : appState ? '사용 준비 완료' : '연결 중'}<span class="version">v0.2.0</span></div>
  </aside>
  <main class="control-main">
    ...
    {:else}
      <section class="about-panel"><span class="brand-mark large"><Icon name="pen" size={36} /></span><h1>My Brush</h1><p>설명을 오래 남기는 화면 드로잉 도구</p><span class="about-version">VERSION 0.2.0</span>
