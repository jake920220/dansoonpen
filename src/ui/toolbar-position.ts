export interface ToolbarPosition { x: number; y: number; collapsed: boolean }
export const DEFAULT_POSITION: ToolbarPosition = { x: 0.5, y: 0, collapsed: false };
export function readPosition(value: string | null): ToolbarPosition {
  try {
    ...
    if (p && Number.isFinite(p.x) && Number.isFinite(p.y) && typeof p.collapsed === 'boolean') {
      return { x: Math.max(0, Math.min(1, p.x)), y: Math.max(0, Math.min(1, p.y)), collapsed: p.collapsed };
    }
