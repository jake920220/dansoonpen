...
| 전체 지우고 앱 조작으로 복귀 | Option+X | Alt+Shift+X |
...
전체 지우기는 필기를 페이드로 지우는 동시에 그리기 모드를 끝내고 이전 앱 조작으로 돌아갑니다. 단축키·도구 모음·트레이에서 같은 동작을 사용합니다. 필기가 없는 상태에서도 그리기 모드를 끝내며, 이미 앱 조작 모드라면 현재 앱의 포커스를 유지합니다.
