...
**v0.2는 로컬 검증 단계입니다.** OS별 실제 검증 범위는 [호환성 기록](docs/testing/compatibility.md)을 확인하세요. Windows 설치·실기 검증과 회의 수신 화면 검증을 완료하기 전까지 두 OS의 정식 지원을 선언하지 않습니다.
...
사각형·원·포인터 강조·세션 저장·이미지 내보내기는 고도화 단계입니다.
