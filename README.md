...
**v0.3은 로컬 검증 단계입니다.** OS별 실제 검증 범위는 [호환성 기록](docs/testing/compatibility.md)을 확인하세요. Windows 설치·실기 검증과 회의 수신 화면 검증을 완료하기 전까지 두 OS의 정식 지원을 선언하지 않습니다.
...
프리셋은 도구·색상·펜 굵기·글자 크기·형광펜 굵기와 불투명도를 함께 저장합니다. 도구막대의 번호 버튼 또는 Shift+1–3으로 불러오세요. 색상창에서 저장할 번호를 고른 뒤 **현재 도구 저장**을 누르면 해당 조합을 덮어씁니다. 이름은 설정창에서 바꿀 수 있습니다. **기본 펜으로**는 저장된 시작 기본값을 불러옵니다.
