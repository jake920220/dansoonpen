...
전체 지우기는 필기를 페이드로 지우는 동시에 그리기 모드를 끝내고 이전 앱 조작으로 돌아갑니다. 단축키·도구 모음·트레이에서 같은 동작을 사용합니다. 모드 전환·삭제 결과는 3.5초간 작은 알림으로 표시합니다. 삭제 직후 메뉴 막대/트레이의 **방금 지운 필기 되돌리기**, 설정창, 그리기 도구막대의 **삭제 복구**로 복원할 수 있습니다. 새 편집이나 undo/redo가 생기면 해당 복구 버튼을 해제해 새 작업을 잘못 취소하지 않습니다. 앱 조작 중 화면 알림은 클릭을 통과시키고 메뉴에서 복구합니다. 필기가 없는 상태에서도 그리기 모드를 끝내며, 이미 앱 조작 모드라면 현재 앱의 포커스를 유지합니다.
