...
텍스트 도구(T)로 이미 작성한 글을 클릭하면 같은 위치·크기·색상으로 다시 편집합니다. 변경을 완료하면 실행 취소 한 번으로 원문을 되돌릴 수 있습니다. Esc는 수정 취소이며, 내용을 모두 비우고 완료하면 해당 글을 삭제합니다. 기본 크기는 40px이고 기존 28px 기본값은 업그레이드 때 40px로 바뀝니다. 별도로 지정한 다른 글자 크기와 직접 만든 프리셋은 유지됩니다.
