...
작성일: 2026-09-08. 상태: 핵심 v0.1 구현·자동 검사·macOS release 빌드·핵심 수동 확인 완료. Windows 실기, IME/전체화면/다중 화면 반복, 회의 수신 및 정량 성능 검증은 대기한다. 추가 기능은 고도화로 연기했다. 아래 전체 검증 기준의 합격을 뜻하지 않으며 실제 결과는 docs/testing에 기록한다.
...
최초 계획 시 폴더는 비어 있었고 Git 저장소가 아니었다. 현재 로컬 main에 기능별 구현이 기록되어 있다. 아래 구조는 계획 당시의 예정 경로이며 실제 구조는 docs/adr/0001-overlay-stack.md를 따른다.
...
후속 결정은 다음 순서로 다룬다. (1) 핵심 흐름·추가 기능 우선순위, (2) 출처 표시 요구에 맞춘 Apache-2.0 + NOTICE 권장안 적용과 저작권자 표시 이름, (3) 최소 지원 OS·CPU와 실제 강의 앱 목록, (4) 기술 실험 결과를 통한 Tauri 확정, (5) 공개 직전 서명·공증 비용과 리모트 연결. 구현 단계에서 Apache-2.0 LICENSE·NOTICE·의존성 고지를 생성하고 macOS 로컬 서명 ZIP을 만들었다. 인증서 구매·Developer ID 공증·리모트 연결·공개 배포는 수행하지 않았다.
